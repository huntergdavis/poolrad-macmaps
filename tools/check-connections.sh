...
OUT="$(mktemp -d "$ROOT/scratch/connections-check.XXXXXX")"
mkdir -p "$OUT/classes"
cat > "$OUT/AndroidManifest.xml" <<'XML'
<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="com.hunterdavis.poolradmacmaps.f58check">
<uses-sdk android:minSdkVersion="26" android:targetSdkVersion="30"/>
<application android:label="F58 controlled check"/>
<instrumentation android:name="name.osher.gil.minivmac.check.ConnectionsCheck" android:targetPackage="com.hunterdavis.poolradmacmaps.ii"/>
</manifest>
XML
javac -cp "$SDK/platforms/android-34/android.jar" -d "$OUT/classes" "$ROOT/tools/ConnectionsCheck.java"
...
"$ADB" -s "$SERIAL" shell am instrument -w com.hunterdavis.poolradmacmaps.f58check/name.osher.gil.minivmac.check.ConnectionsCheck | tee "$OUT/result.txt"
mkdir -p "$OUT/screenshots"
"$ADB" -s "$SERIAL" exec-out run-as "$PACKAGE" tar -C files -cf - f58-check | tar -C "$OUT/screenshots" -xf -
grep -q '^PASS empty graph' "$OUT/result.txt"
echo "Evidence: $OUT"
